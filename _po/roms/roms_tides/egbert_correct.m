function [pf_,pu_,t0,phase_mkB_]=egbert_correct(mjd,hr,minute,second,comps)
%---------------------------------------------------------------------
...
% added:
%   m4 ms4 mn4 (present in the tpxo Atlantic Ocean solution)
%---------------------------------------------------------------------
%
rad=pi/180.0;
deg=180.0/pi;
...
% return only data from selected comps:
C={'m2','s2','n2','k2','k1','o1','p1','q1','mf','mm','m4','ms4','mn4'};

inds=zeros(size(comps));
for i=1:length(comps)
  for j=1:length(C)
    if isequal(comps{i},C{j})
      inds(i)=j;
      break;
    end
  end
...
pf_=nan*zeros(size(comps));
pu_=nan*zeros(size(comps));
phase_mkB_=nan*zeros(size(comps));
for i=1:length(inds)
  if inds(i)>0
    pf_(i)=pf(inds(i));
    pu_(i)=pu(inds(i));
    phase_mkB_(i)=phase_mkB(inds(i));
  end
end
