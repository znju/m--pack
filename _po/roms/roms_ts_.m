...
for ii=1:length(vin)
  if     isequal(vin{ii},'time')
    itime    = vin{ii+1};
  elseif isequal(vin{ii},'xi')
    ixi_rho  = vin{ii+1};
    ixi_u    = vin{ii+1};
    ixi_v    = vin{ii+1};
  elseif isequal(vin{ii},'eta')
    ieta_rho = vin{ii+1};
    ieta_u   = vin{ii+1};
    ieta_v   = vin{ii+1};
  elseif isequal(vin{ii},'station')
    istation  = vin{ii+1};
  end
end
...
use(file,varname,'+time+',itime,'xi+',ixi,'eta+',ieta,'s_',is_);
%v=use(file,varname,args{:});
varargin
