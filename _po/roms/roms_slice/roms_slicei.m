function [x,y,z,v,mask]=roms_slicei(fname,varname,ind,t,varargin)
%ROMS_SLICEI   Make ROMS slice across eta direction (i=const)
%
%   Syntax:
%      [X,Y,Z,V,M] = ROMS_SLICEI(FILE,VARNAME,I,T,VARARGIN)
...
%      i = 50;
%      t = 10;
%      [x,y,z,v,mask]=roms_slicei(file,varname,i,t);
%      figure
%      surf(x,y,z,v);
%      figure
%      pcolor(y,z,v)
...
%      [x,y,z,v,mask]=roms_slicei(file,varname,i);
%      figure
%      plot3(x,y,-v)
%
...
% check ind:
ninds=0;
if ismember('xi_rho',dnames),   ninds=vdims('xi_rho');
elseif ismember('xi_u',dnames), ninds=vdims('xi_u');
elseif ismember('xi_v',dnames), ninds=vdims('xi_v');
end

if ind>ninds
  fprintf(1,':: %s :: ind = %g exceeds xi dimension (%g)\n',mfilename,ind,ninds);
  return
end
...
for i=1:length(dnames)
  if isequal(dnames{i},'time') | isequal(dnames{i},'ocean_time')
    s=[s ',' num2str(t)];
  elseif isequal(dnames{i},'xi_rho') | isequal(dnames{i},'xi_u') | isequal(dnames{i},'xi_v')
    s=[s ',' num2str(ind)];
  else
    s=[s ',:'];
  end
...
[x,y,h,mask]=roms_grid(grd,varname(1),'xi',ind); mask1d=mask;
...
if plt
  ...
  if prod(size(v))==length(v) % 1d
    ...
    if plt==3
      ...
      s=pcolor(y,z,vv); set(s,'edgecolor','none');
      hold on
      h(mask1d==0)=nan;
      zeta(mask1d==0)=nan;
      plot(y,-h);
