function blin(task,point)
%BLIN   Add broken line to current axis
%   When BLIN is called clik with left mouse button on axis to choose
...
if isequal(task,'create');
  ...
  if n>1
    ...
    set(s,'tag','blin');
    set(aux,'userdata',s);
    set(aux,'tag','blin_aux');
    set(gcf,'windowbuttondownfcn','blin(''get'')');
...
if isequal(task,'get');
  ...
  % find point to move:
  if isequal(get(gco,'tag'),'blin_aux')
    x=get(gco,'xdata');
    y=get(gco,'ydata');
    ...
    if isequal(get(gcf,'SelectionType'),'normal')
      motion=['blin(''move'',',num2str(i),')'];
      set(gcf,'windowbuttonMotionfcn',motion);
      set(gcf,'windowbuttonUpfcn','blin(''stop'')');
  ...
  % ---------------------------------------------------- add new point:
  elseif isequal(get(gco,'tag'),'blin')
    ...
    elseif isequal(get(gcf,'SelectionType'),'normal')
      ...
      yi=cp(1,2);
      point=[xi,yi];
       motion=['blin(''move'',[',num2str(point),'])'];
      set(gcf,'windowbuttonMotionfcn',motion);
      set(gcf,'windowbuttonUpfcn','blin(''stop'')');
...
if isequal(task,'move');
  ...
  if isequal(get(gco,'tag'),'blin') & isequal(get(gcf,'SelectionType'),'normal')
    % move all
    x=get(gco,'xdata');
    y=get(gco,'ydata');
    aux=get(gco,'userdata');
...
function xys=re_blin(xy)
dt=.01;
n=size(xy,2);
t=1:n;
ts=1:dt:n;
xys(1,:)=interp1(t,xy(1,:),ts);
xys(2,:)=interp1(t,xy(2,:),ts);
