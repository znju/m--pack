...
[b,status] = nc_getall;
if ( status >= 0 )
	msg = sprintf ( '%s:  nc_getall succeeded on Test 1 when it should have failed.\n' );
	error ( msg );
end
...
%
% Test 3:  empty file
% result should just be an empty structure
fprintf ( 1, 'Test 3\n' );
[nb, status] = nc_getall ( ncfile );
if ( status < 0 )
	msg = sprintf ( '%s:  nc_getall failed on Test 3.\n' );
	error ( msg );
end
if ~isstruct ( nb )
	error ( 'Test 3 result should have been a structure\n' );
end
if length(nb) ~= 0
	error ( 'Test 3 result should have been an empty structure\n' );
end
...
%
% Test 4:  just one dimension
% Results should still be an empty structure.
fprintf ( 1, 'Test 4\n' );
status = nc_add_dimension ( ncfile, 'x', 6 );
[nb, status] = nc_getall ( ncfile );
if ( status < 0 )
	msg = sprintf ( '%s:  nc_getall failed on Test 4.\n' );
	error ( msg );
end
if ~isstruct ( nb )
	error ( 'Test 4 result should have been a structure\n' );
end
if length(nb) ~= 0
	error ( 'Test 4 result should have been an empty structure\n' );
end
...
[nb, status] = nc_getall ( ncfile );
if ( status < 0 )
	msg = sprintf ( '%s:  nc_getall failed on Test 5.\n' );
	error ( msg );
end
if ~isfield(nb, 'x' )
	msg = sprintf ( '%s:  nc_getall did not return a field x.\n' );
	error ( msg );
end
if ~isfield(nb.x, 'data' )
	msg = sprintf ( '%s:  nc_getall did not return a field x.data.\n' );
	error ( msg );
end
...
if length(nb.global_atts) ~= 6
	msg = sprintf ( '%s:  nc_getall did not return 6 global atts.\n' );
	error ( msg );
end
...
if ~isfield(nb, 'y' )
	msg = sprintf ( '%s:  nc_getall did not return a field y.\n' );
	error ( msg );
end
if length(nb.y) ~= 8
	msg = sprintf ( '%s:  nc_getall did not return all the attributes of y.\n' );
	error ( msg );
end
