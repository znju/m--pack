...
try
	status = nc_add_recs;
	error ( [ test_id ':' ' nc_add_recs succeeded on no inputs, should have failed'] );
end
fprintf ( '%s:  passed\n', test_id );
...
test_id = 'Test 4';
status = nc_add_recs ( ncfile, struct([]) );
if ( status >= 0 )
...
input_data.a = [3 4];
input_data.b = [5 6];
status = nc_add_recs ( ncfile, input_data );
if ( status >= 0 )
...
status = nc_add_recs ( ncfile, input_data, 'bad_unlim_dim_name' );
if ( status >= 0 )
	error ( 'nc_add_recs succeeded with a badly named unlimited dimension, should have failed' );
end
fprintf ( '%s:  passed\n', test_id );
...
status = nc_add_recs ( ncfile, input_buffer );
if ( status < 0 )
	error ( 'nc_add_recs failed' );
end
...
fprintf ( 1, 'NC_ADD_RECS succeeded\n' );
