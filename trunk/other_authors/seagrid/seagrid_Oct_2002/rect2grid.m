...
%  NOTE: When we solve via FFT, the aspect-ratio of the
%  rectangle needs to be accomodated by adjusting size
%  [m n] so that the grid-cells are roughly square.
%  If the aspect-ratio is 1.72, for example, then
%  the size of our solution plane (u and v) should
%  be of ratio 100-by-172, not the 100-by-100
%  used at present.  (We can use the Matlab "rat"
%  function to help select the actual size.)
%  So, use the aspect ratio to determine the number
%  of interpolated points around the perimeter,
%  populate a solution matrix of similar ratio,
%  solve, interpolate to to the array size we
%  actually want.
