function [x,y,z,v,mask]=roms_slicek(fname,varname,ind,t,varargin)
...
%      k = 15;
%      t = 10;
%      [x,y,z,v,mask]=roms_slicek(file,varname,k,t);
%      figure
%      surf(x,y,z,v);
%      figure
%      pcolor(x,y,v)
...
%      [x,y,z,v,mask]=roms_slicek(file,varname,'whatever...',t);
%      figure
%      pcolor(x,y,v)
%
%      varname = 'h';       % 2-d array (no t or k required)
%      [x,y,z,v,mask]=roms_slicek(file,varname);
%      figure
%      pcolor(x,y,-v)
%
...
% check ind:
ninds=0;
if ismember('s_rho',dnames),   ninds=vdims('s_rho');
elseif ismember('s_w',dnames), ninds=vdims('s_w');
end

if ninds>0 &ind>ninds
  fprintf(1,':: %s :: ind = %g exceeds s dimension (%g)\n',mfilename,ind,ninds);
  return
end
...
for i=1:length(dnames)
  if isequal(dnames{i},'time') | isequal(dnames{i},'ocean_time')
    s=[s ',' num2str(t)];
  elseif isequal(dnames{i},'s_rho') | isequal(dnames{i},'s_w')
    s=[s ',' num2str(ind)];
  else
    s=[s ',:'];
  end
