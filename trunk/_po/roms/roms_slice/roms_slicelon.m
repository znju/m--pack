function [x,y,z,v,labels]=roms_slicelon(file,varname,lon,t,varargin)
...
%      params.hc  = 5;
%      params.N   = 20;
%      [x,y,z,v,labels]=roms_slicelon(file,varname,lon,t,'s_params',params);
%      figure
%      surf(x,y,z,v);
%      figure
%      pcolor(y,z,v)
...
%      [x,y,z,v,labels]=roms_slicelon(file,varname,lon,t);
%      figure
%      plot3(x,y,v)
%
%      varname = 'h';       % 2-d array
%      [x,y,z,v,labels]=roms_slicelon(file,varname,lon);
%      figure
%      plot3(x,y,-v)
%
...
if ~isnumber(lon,1) & ~isnumber(lon,2)
  disp('» bad lon input arguments');
  return
end
...
x = lon;
y = linspace(min(min(Y)),max(max(Y)),nLON);
...
if isS_rho | isS_w
  ...
  wb = waitbar(0,'slice lon, Please wait...');
  for k =1:K
